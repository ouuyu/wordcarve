...
<template>
  <div class="relative">
    ...
    <div
      v-if="isMiniMode"
      class="w-full flex items-center gap-3 rounded-md px-4 py-2 transition-all hover:bg-gray-100"
    ...
    <!-- Normal 模式 -->
    <div v-else class="w-full flex flex-col items-start md:flex-row">
      <!-- PC端左侧导航栏 -->
          ...
          <div v-if="word.exchange" class="mb-6">
            ...
            <div class="grid grid-cols-2 gap-2">
              <div v-if="word.exchange.s" class="font-medium">
                <span class="text-sm text-gray-500">复数: </span>{{ word.exchange.s }}
              </div>
              <div v-if="word.exchange.p" class="font-medium">
                <span class="text-sm text-gray-500">过去式: </span>{{ word.exchange.p }}
              </div>
              <div v-if="word.exchange.d" class="font-medium">
                <span class="text-sm text-gray-500">过去分词: </span>{{ word.exchange.d }}
              </div>
              <div v-if="word.exchange.i" class="font-medium">
                <span class="text-sm text-gray-500">现在分词: </span>{{ word.exchange.i }}
              </div>
              <div v-if="word.exchange['3']" class="font-medium">
                <span class="text-sm text-gray-500">第三人称单数: </span>{{ word.exchange['3'] }}
              </div>
              <div v-if="word.exchange.r" class="font-medium">
                <span class="text-sm text-gray-500">比较级: </span>{{ word.exchange.r }}
              </div>
              <div v-if="word.exchange.t" class="font-medium">
                <span class="text-sm text-gray-500">最高级: </span>{{ word.exchange.t }}
              </div>
            </div>
          </div>
        </div>
